...
void pagingSimulation() {
    ...
    // Randomly map logical pages to physical frames
    for (int i = 0; i < MEMORY_SIZE / PAGE_SIZE; i++) {
        pageTable[i] = (rand() % (MEMORY_SIZE / PAGE_SIZE)) * PAGE_SIZE; 
    }
    ...
    int physicalAddress = pageTable[pageNumber] + offset; // Correct mapping
