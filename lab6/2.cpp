...
struct Process {
    int id, arrival_time, burst_time, remaining_time, completion_time, waiting_time, turnaround_time, priority;
};
...
void roundRobinScheduling(vector<Process>& processes, int time_quantum) {
    ...
    while (completed < processes.size()) {
        ...
        while (!readyQueue.empty()) {
            priorityQueue.push_back(readyQueue.front());
            readyQueue.pop();
        }
        
        sort(priorityQueue.begin(), priorityQueue.end(), [&](int a, int b) {
...
void calculateAverages(vector<Process>& processes) {
    ...
    cout << "\nProcess\tAT\tBT\tP\tCT\tTAT\tWT" << endl;
    for (const auto& process : processes) {
        total_tat += process.turnaround_time;
        total_wt += process.waiting_time;
        cout << process.id << "\t" << process.arrival_time << "\t" << process.burst_time << "\t" << process.priority << "\t" << process.completion_time << "\t" 
